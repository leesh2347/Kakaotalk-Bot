...
def handle_gym(sender, chat, args=None):
    ...
    for skill in player1pok.get("skills", []):
        ...
        player1pp[skill] = skill_data.get("pp", 10) if skill_data else 10
    
    player2pp = {}
    for skill in player2pok.get("skills", []):
        skill_data = read_json(f"기술/{skill}")
        player2pp[skill] = skill_data.get("pp", 10) if skill_data else 10
...
def battle_loop(chat, sender):
    ...
    while turn < max_turns:
        ...
        if skill1_data:
            player1spd += skill1_data.get("priority", 0) * 2
        if skill2_data:
            player2spd += skill2_data.get("priority", 0) * 2
